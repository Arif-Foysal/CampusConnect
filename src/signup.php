...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  $marketing = 0;
  if (isset($_POST['marketing_accept'])) {
    # code...
    $marketing = 1;
  }
  // $marketing = isset($_POST['marketing_accept']); 

  $usertype = "student"; //Default Users type when they sign in
  // Check whether this student_id exists
  $existSql = "SELECT * FROM `Users` WHERE student_id = '$student_id'";
  $result = mysqli_query($conn, $existSql);
  $numExistRows = mysqli_num_rows($result);
  if ($numExistRows > 0) {
    // $exists = true;
    $showError = "student_id Already Exists";
  } else {
    // $exists = false; 
    if (($password == $confirm_password)) {
      $hash = password_hash($password, PASSWORD_DEFAULT);
      $sql = "INSERT INTO `Users` ( `student_id`, `password`,`fname`,`lname`,`rcvEmails`, `user_type`) VALUES ('$student_id', '$hash', '$fname', '$lname','$marketing', '$usertype')";
      $result = mysqli_query($conn, $sql);
      if ($result) {
        $showAlert = true;
        // echo "success";
        {
          $sql = "SELECT user_id, student_id, password FROM Users WHERE student_id = ?";
...
<head>
  ...
  <script>
    function validateForm() {
      ...
      document.getElementById("student_id_err").innerText = "";
      document.getElementById("password_err").innerText = "";
      document.getElementById("confirm_password_err").innerText = "";

      const student_id = document.forms["signupForm"]["student_id"].value;
      const password = document.forms["signupForm"]["password"].value;
      const confirmPassword = document.forms["signupForm"]["confirm_password"].value;

      // Validate student_id
      if (student_id == "") {
        document.getElementById("student_id_err").innerText = "student_id cannot be empty";
        isValid = false;
      }
      ...
      <form name="signupForm" action="" method="post" class="mt-8 grid grid-cols-6 gap-6 "
        ...
        <div class="col-span-6">
          ...
          <input type="text" id="student_id" name="student_id"
            class="mt-1 w-full rounded-lg border-gray-400 bg-gray-50 text-sm text-gray-700 shadow-sm focus:border-transparent" value="" />
          <span id="student_id_err" class="text-red-500 text-sm"></span>
        </div>
        ...
        <div class="col-span-6">
          <label for="marketing_accept" class="flex gap-4">
            ...
            <span class="text-sm text-gray-700">
              I want to receive Emails about anouncements and updates
            </span>
          </label>
        </div>
        ...
        <div class="col-span-6 justify-center sm:flex sm:items-center sm:gap-4">
          <div class="flex flex-col w-full max-w-xs gap-y-5">
            <button type="submit"
              class="inline-block shrink-0 rounded-md border border-orange-600 bg-orange-600 px-12 py-3 text-lg font-semibold text-white transition hover:bg-transparent hover:text-orange-600 focus:outline-none focus:ring active:text-orange-600">
              Create an account
            </button>

            <button
              class="bg-white flex items-center text-gray-700 justify-center rounded-md gap-x-3 text-sm sm:text-base hover:bg-gray-100 duration-300 transition-colors border px-8 py-2.5">
